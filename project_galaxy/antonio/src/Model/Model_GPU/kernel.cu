	...
	int n_particles)
{
	...
	// So:
	float xi = positionsGPU[i].x;
	float yi = positionsGPU[i].y;
	float zi = positionsGPU[i].z;

	for (int j = 0; j < n_particles; j++)
	{
		if(i != j)
		{
			const float diffx = positionsGPU[j].x - xi;
			const float diffy = positionsGPU[j].y - yi;
			const float diffz = positionsGPU[j].z - zi;
